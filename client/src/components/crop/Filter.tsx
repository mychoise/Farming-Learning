...
const Filter = () => {
    const FilterOptions = [
        {
            name: "Season",
            options: ["Spring(May-Aug)", "Summer(Sept-Nov)", "Autumn(Dec-Feb)", "Winter(Mar-Apr)"]
        },
        {
            name: "Difficulty",
    ...
    const [selected, setSelected] = useState<string[]>(FilterOptions[0].options)
    ...
    return (
        <div className="flex flex-col ">
            ...
            <div className="bg-white border mt-4 h-42.5 border-gray-200 rounded-xl font-[Inter] p-4 max-w-xs">
                <h2 className="text-xs font-bold uppercase tracking-widest text-gray-800 mb-3">Growth Season</h2>
                <div className="flex mt-2 flex-col gap-2">
                    {FilterOptions[0].options.map((option) => {
                        const checked = selected.includes(option)
                        return (
                            <label key={option} className="flex items-center gap-3 cursor-pointer text-sm text-gray-800">
                                <span
                                ...
                                >
                                    {checked && (
                                        <svg width="12" height="9" viewBox="0 0 13 10" fill="none">
                                            <polyline points="1.5,5 5,8.5 11.5,1.5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                    )}
                                </span>
                                {option}
              ...
              <div className="bg-white border mt-4 h-42.5 border-gray-200 rounded-xl font-[Inter] p-4 max-w-xs">
                <h2 className="text-xs font-bold uppercase tracking-widest text-gray-800 mb-3">Skill Level</h2>
                <div className="flex mt-2 flex-col gap-2">
      ...
      <p className="text-gray-700 z-10 relative text-sm leading-relaxed">Planting during the rainy season ensures better germination and growth.</p>
    </div>
