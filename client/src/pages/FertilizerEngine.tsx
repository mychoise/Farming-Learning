...
const data = queryClient.getQueryData(["organicFertilizerResult"]);
...
const total = data?.nutrientsNeeded.N_kg + data?.nutrientsNeeded.P_kg + data?.nutrientsNeeded.K_kg || 0;
  const maxN = total || 200, maxP =  total || 100, maxK = total || 220;
  const N=120, P=60, K=30;
    ...
    <div className="min-h-screen bg-stone-100 font-[font4]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-50">
          ...
          {/* Right: Results */}
          {generated?(<div className="-mt-[40%] -ml-[10%] space-y-5 w-122.5">
            {/* NPK Targets */}
            ...
            <div>
              ...
              <div className={`grid grid-cols-2 gap-5 w-[750px] ${animating ? "opacity-50" : "fade-in"}`}>
 {FERTILIZERS.map((f) => (
    <StrategyCard
      key={f.name}
      label="Organic Supply"
      name={f.name}
      amount={data?.fertilizers?.[f.key] || f.defaultAmount}
      unit={f.unit}
      icon={f.icon}
    ...
    />
            ...
            <div className="bg-white w-175 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center h-64 border-2 border-dashed border-stone-200">
